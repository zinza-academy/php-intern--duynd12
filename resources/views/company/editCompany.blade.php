    ...
    <div class="pt-3 pr-0.5 pb-3 pl-6 flex flex-col items-start gap-5  left-0 top-20" style="width: 1444px;height:952px">
        <div class="flex items-end flex-row h-20 gap-2 " style="width: 650px">
            <div class="flex flex-col items-start gap-2 h-20 w-80">
                ...
                <input value="{{old('name_company') ? old('name_company') : $data['name_company']}}" id="name" class="border-2 bg-white rounded-md h-10 w-full py-1 pl-3 focus:outline-none focus:border-sky-500 focus:ring-sky-500" name="name_company" placeholder="Name"/>
                @error('name')
            ...
            <img src="{{URL::asset('storage/images/logo/').'/'.$data['logo']}}" alt="" id="avatar" class="w-24 h-24 bg-center rounded-full">
        </div>
        <div class="flex items-end flex-row h-20 gap-2" style="width: 980px">
            <div class="flex flex-col items-start gap-2 h-20 w-80">
                <label for="address">Address</label>
                <input value="{{old('address') ? old('address') :$data['address']}}" class="border-2 bg-white rounded-md h-10 w-full py-1 pl-3 focus:outline-none focus:border-sky-500 focus:ring-sky-500" name="address" id="address" placeholder="Address"/>
                <span id ="msg_password" style="color:red;display:none">Mật khẩu không khớp nhau</span>
            </div>  
            <div class="flex flex-col items-start gap-2 h-20 w-80">
                <label for="">Max_users</label>
                <input value="{{old('max_users') ? old('max_users') :$data['max_users']}}" type="text"  class="border-2 bg-white rounded-md h-10 w-full py-1 pl-3 focus:outline-none focus:border-sky-500 focus:ring-sky-500" name="max_users" placeholder="Max_users"/>
            </div>
        </div>
        <div class="flex items-end flex-row h-20 gap-2" style="width: 980px">
            <div class="flex flex-col items-start gap-2 h-20 w-80">
                <label for="Expired_at">Exprired at</label>
                <input type="datetime-local" value="{{old('expired_time') ? old('expired_time') :$data['expired_time']}}" id ='Expired_at' class="border-2 bg-white rounded-md h-10 w-full py-1 pl-3 focus:outline-none focus:border-sky-500 focus:ring-sky-500" name="expired_time" placeholder="Expired">
            ...
            <div class="flex flex-col items-start gap-2 h-20 w-80">
                ...
                <select  id ='active'  class="border-2 bg-white rounded-md h-10 w-full py-1 pl-3 focus:outline-none focus:border-sky-500 focus:ring-sky-500" name="active" placeholder="">
                    @foreach(App\Constants\StatusConstants::COMPANY_ACTIVE as $status)
                        <option {{$data['active'] === $status ? 'selected' : ''}} value="{{$status}}">{{$status}}</option>
                    @endforeach  
                </select>
            </div>
            @error('dob')
